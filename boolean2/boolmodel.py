...
class BoolModel(Parser):
    ...
    def report_cycles(self):
        ...
        if size == 0:
            print(
                "No cycle or steady state could be detected from the %d states" % len(
                    self.states))
        elif size == 1:
            print("Steady state starting at index %s -> %s" % (
...
if __name__ == '__main__':
    ...
    print(
        'States %s -> Detect cycles %s' % (states, util.detect_cycles(states)))
