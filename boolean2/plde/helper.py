...
def prop_func(node, indexer, par):
    ...
    try:
        ...
        text = ' prop( r=%s, rc=%s ) - %s ' % (
        par[node].r, par[node].rc, nconc)
    except Exception as exc:
        msg = "error creating proportion function for node %s -> %s" % (
        node, exc)
        raise Exception(msg)
    return text
            ...
            if 'default' in kwds:
                ...
                for label in labels:
                    if label not in data[node]:
                        raise KeyError("could not find parameter %s['%s']" % (
                        node, label))
...
def read_parameters(fname):
    ...
    # check file size 
    assert len(
        lines) > 2, "file '%s' needs to have more than two lines" % fname
    ...
    def coltest(elems):
        ...
        if size != colnum:
            raise Exception(
                "column number mismatch expected %d, found %s, at line '%s'" % (
                size, colnum, ', '.join(elems)))
        return True
