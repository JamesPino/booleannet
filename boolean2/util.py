...
def bool_to_tuple(value):
    """
    Converts a boolean2 value to concentration, decay, threshold triplets
    """
...
def check_case(nodes):
    ...
    if len(upcased) != len(nodes):
        error(
            'some node names are capitalized in different ways -> %s!' % list(
                nodes))
...
class Collector(object):
    ...
    def get_averages(self, normalize=True):
        ...
        for node in self.store:
            all_tmp = self.store[node]
            size = float(len(all_tmp))

            # this sums lists!
            def listadd(xlist, ylist):
                return [x + y for x, y in zip(xlist, ylist)]

            values = reduce(listadd, all_tmp)
